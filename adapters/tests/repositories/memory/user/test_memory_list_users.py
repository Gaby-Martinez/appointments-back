...
async def test_multiple_users(user_repository, sample_user):
    users = [
        sample_user,
...
async def test_repository_operation_exception_on_get_all_doctors(user_repository):
    user_repository.users = None

    with pytest.raises(RepositoryOperationException) as exc_info:
        await user_repository.get_all_doctors()

    assert "User" in str(exc_info.value)
    assert "get_all_doctors" in str(exc_info.value)


async def test_repository_operation_exception_on_get_all_patients(user_repository):
    user_repository.users = None

    with pytest.raises(RepositoryOperationException) as exc_info:
        await user_repository.get_all_patients()

    assert "User" in str(exc_info.value)
    assert "get_all_patients" in str(exc_info.value)
